...
export function getJpegDimensions(source) {
	if (source.length < 4 || source[0] !== 0xff || source[1] !== 0xd8) throw new Error("The social image must be a valid JPEG.");

	let offset = 2;
	while (offset < source.length) {
		if (source[offset] !== 0xff) throw new Error("The social image must be a valid JPEG.");
		...
		if (segmentLength < 2 || offset + segmentLength > source.length) throw new Error("The social image must be a valid JPEG.");
		...
		if (isStartOfFrame) {
			if (segmentLength < 8) throw new Error("The social image must be a valid JPEG.");
...
export function getAttribute(element, attribute) {
	const attributePattern = new RegExp(`\\b${escapeRegularExpression(attribute)}=(["'])([\\s\\S]*?)\\1`, "i");
