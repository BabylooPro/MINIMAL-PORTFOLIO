...
export function getSiteControllerScript(html) {
	...
	if (controllerScripts.length !== 1) throw new Error("The production HTML must contain exactly one site controller script.");

	const [controllerScript] = controllerScripts;
	const src = getAttribute(controllerScript, "src");
	if (!isModuleScript(controllerScript) || !src) throw new Error("The site controller must be a module script with a src attribute.");
...
function parseStructuredData(indexHtml, route) {
	...
	if (structuredDataScripts.length !== 1) throw new Error(`The ${route.kind} page must contain exactly one JSON-LD script.`);

	const [structuredDataScript] = structuredDataScripts;
	const content = structuredDataScript.slice(structuredDataScript.indexOf(">") + 1, structuredDataScript.lastIndexOf("</script>"));
...
export function validatePageHtml({ page, route, indexHtml, siteControllerSrc, pageAlternateLinks, languageSwitcherLinks, siteName, alternateLinksMarker, pageMetadataMarker, structuredDataMarker }) {
	...
	if (/\sstyle\s*=/i.test(indexHtml)) throw new Error(`The ${route.kind} page must not contain inline styles.`);
	...
	if (!indexHtml.includes(`<title>${escapeHtml(page.title)}</title>`)) throw new Error(`The ${route.kind} page has an incorrect document title.`);

	assertSingleMetadata(getMetaElements(indexHtml, "robots"), page.indexable ? "index,follow,max-image-preview:large" : "noindex,follow", "robots directive");
	assertSingleMetadata(getMetaElements(indexHtml, "description"), page.description, "description metadata");
	assertSingleMetadata(getMetaElements(indexHtml, "twitter:card"), "summary_large_image", "Twitter card metadata");
	assertSingleMetadata(getMetaElements(indexHtml, "twitter:title"), page.title, "Twitter title metadata");
	assertSingleMetadata(getMetaElements(indexHtml, "twitter:description"), page.ogDescription, "Twitter description metadata");
	assertSingleMetadata(getMetaElements(indexHtml, "twitter:image"), page.socialImage.url, "Twitter image metadata");
	assertSingleMetadata(getMetaElements(indexHtml, "twitter:image:alt"), page.socialImage.alt, "Twitter image alt metadata");
	...
	if (
		!languageSwitcherLinks.every((link) => {
			...
			return getAnchorElements(indexHtml).some((anchor) => getAttribute(anchor, "href") === href && getAttribute(anchor, "hreflang") === link.hreflang);
		})
	) {
