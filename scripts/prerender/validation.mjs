...
import { getAttribute, getJpegDimensions, getScriptElements, hasAttribute, isModuleScript } from "./html.mjs";
...
function normalizePublicAssetPath(assetPath) {
	if (!assetPath.startsWith("/") || assetPath.startsWith("//")) throw new Error(`The site controller has an invalid public path: ${assetPath}`);
...
function toDistPath(assetPath, distDirectory) {
	...
	if (!outputPath.startsWith(distPrefix)) throw new Error(`The site controller asset is outside dist: ${assetPath}`);
	return outputPath;
}

export function getLocaleRedirectScript(html) {
	const redirectScripts = getScriptElements(html).filter((scriptElement) => hasAttribute(scriptElement, "data-locale-redirect"));
	if (redirectScripts.length !== 1) throw new Error("The root page must contain exactly one locale redirect script.");

	const [redirectScript] = redirectScripts;
	if (getAttribute(redirectScript, "src")) throw new Error("The locale redirect script must be inline.");

	return redirectScript;
}

export function getReactEntryScript(html) {
	const reactEntryScripts = getScriptElements(html).filter((scriptElement) => hasAttribute(scriptElement, "data-react-entry"));
	if (reactEntryScripts.length !== 1) throw new Error("The production HTML must contain exactly one React development entry.");

	const [reactEntryScript] = reactEntryScripts;
	if (!isModuleScript(reactEntryScript) || !getAttribute(reactEntryScript, "src")) throw new Error("The React development entry must be a module script with a src attribute.");
}

export function readSiteControllerAssets({ manifest, controllerSrc, siteControllerEntry, distDirectory }) {
	const controller = manifest[siteControllerEntry];
	if (!controller || typeof controller.file !== "string") throw new Error("The Vite manifest does not contain the site controller entry.");

	const controllerAssetPath = normalizePublicAssetPath(controllerSrc);
	if (controller.file !== controllerAssetPath) throw new Error("The Vite manifest and the site controller script disagree.");
	...
	function visit(entryName) {
		...
		if (!entry || typeof entry.file !== "string") throw new Error(`The Vite manifest import is missing: ${entryName}`);
		if (entry.file.endsWith(".js")) assetPaths.add(entry.file);

		for (const importedEntry of entry.imports ?? []) visit(importedEntry);
		for (const dynamicImport of entry.dynamicImports ?? []) visit(dynamicImport);
	}
...
export async function removeUnusedJavaScriptFiles({ distDirectory, allowedFiles }) {
	...
	await Promise.all(files.map(async (filePath) => {
		if (filePath.endsWith(".js.map")) { await unlink(filePath); return; }
		if (filePath.endsWith(".js") && !allowedFiles.has(filePath)) await unlink(filePath);
	}));
}
