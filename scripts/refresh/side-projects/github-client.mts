...
export async function fetchRepositories({ githubUser, cache, fetchImpl, timeoutMs }) {
	...
	try {
		...
		if (response.status === 304) {
			if (!cache) throw createPermanentError("GitHub returned 304 without a usable Side Projects cache.");
		...
		try {
			...
			if (controller.signal.aborted || error?.name === "AbortError") throw createTemporaryError(`GitHub request timed out after ${timeoutMs}ms.`);
			throw createPermanentError("GitHub returned invalid JSON for Side Projects.");
		}

		if (!Array.isArray(repositories)) throw createPermanentError("GitHub returned an incompatible Side Projects response.");

		return { githubUser, repositories, etag: response.headers.get("etag"), fromCache: false };
	} catch (error) {
		if (error instanceof SideProjectsError) throw error;
		if (controller.signal.aborted || error?.name === "AbortError") throw createTemporaryError(`GitHub request timed out after ${timeoutMs}ms.`);
