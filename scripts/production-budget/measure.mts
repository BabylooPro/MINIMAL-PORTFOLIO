...
async function measureFile(filePath) {
	...
	return { filePath, name: path.basename(filePath), rawBytes: source.byteLength, gzipBytes: gzipBytes(source), source };
}
...
export function extractInlineScript(html, attribute) {
	const scripts = getScriptElements(html).filter((script) => hasAttribute(script, attribute) && !getAttribute(script, "src"));
...
function getCanonicalOrigin(html) {
	const canonical = getElements(html, "link").find((element) => getAttribute(element, "rel")?.split(/\s+/).includes("canonical"));
...
export function getCssResourceReferences(source) {
	...
	for (const match of source.matchAll(pattern)) {
		...
		if (url) references.push({ kind: "css url", url, isFont: /@font-face[\s\S]*$/i.test(source.slice(0, match.index)) });
	}
...
export function classifyResourceReference(reference, siteOrigin, allowedOrigins: string[] = []) {
	...
	if (url.startsWith("#") || (!url.includes(":") && !url.startsWith("//"))) return { ...reference, scope: "relative", isThirdParty: false };
...
function getLargestFile(files) {
	return files.reduce((largestFile, file) => (!largestFile || file.rawBytes > largestFile.rawBytes ? file : largestFile), null);
}
...
export async function measureProductionOutput({
	...
	const [javascript, css, previews, html] = await Promise.all([
		...
		Promise.all(htmlPaths.map(async (filePath) => {
			...
			return {
				...
				group: isPrimaryRoute(routeFromOutputPath(relativePath)) ? "primary" : "secondary",
	...
	const socialImage = (await pathExists(socialImagePath)) ? await measureFile(socialImagePath) : null;
	...
	const resourceReferences = [
		...
		...javascript.flatMap((file) => getJavaScriptResourceReferences(file.source.toString("utf8"))),
	...
	return {
		...
		executableJavascript: {
			rootGzipBytes: (themeBootstrap?.gzipBytes ?? 0) + (localeRedirect?.gzipBytes ?? 0) + (controller?.gzipBytes ?? 0),
		...
		architecture: {
			reactRuntimeBytes: reactRuntimeFiles.length === 0 ? 0 : getTotalBytes(reactRuntimeFiles),
			reactRuntimeFiles,
		...
		resources: {
			...
			externalFonts: resourceReferences.filter((reference) => reference.isThirdParty && reference.isFont),
		},
	};
}
