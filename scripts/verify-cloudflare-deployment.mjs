...
if (!siteArgument || extraArguments.length > 0) {
	throw new Error("Usage: pnpm verify:cloudflare <https://maxremy.dev>");
}

const siteUrl = new URL(siteArgument);

if (siteUrl.protocol !== "https:" || siteUrl.pathname !== "/" || siteUrl.search || siteUrl.hash) {
	throw new TypeError("The site URL must be an HTTPS origin without a path, query, or fragment.");
}
...
function requireHeader(response, name, expectedValue) {
	...
	if (!value?.includes(expectedValue)) {
		throw new Error(`Expected ${name}: ${expectedValue}. Received: ${value ?? "missing"}.`);
	}
}
...
async function requireLocalizedNotFound(pathname, expectedText) {
	...
	if (response.status !== 404) {
		throw new Error(`Expected ${pathname} to return 404. Received: ${response.status}.`);
	}

	if (!body.includes(expectedText)) {
		throw new Error(
			`The ${pathname} response does not contain the expected localized 404 page.`,
		);
	}
}
...
if (!homepageResponse.ok) {
	throw new Error(`Expected the homepage to return 2xx. Received: ${homepageResponse.status}.`);
}
...
if (!mediaSourceMatch?.[1]) {
	throw new Error("The homepage does not contain a Proof Work MP4 source.");
}

const mediaUrl = new URL(mediaSourceMatch[1], siteUrl);

if (mediaUrl.protocol !== "https:" || mediaUrl.origin === siteUrl.origin) {
	throw new Error("Proof Work MP4 files must be served from a separate HTTPS media origin.");
}
...
if (mediaResponse.status !== 206 || !mediaResponse.headers.get("content-range")) {
	throw new Error(
		`Expected a byte-range response from ${mediaUrl}. Received: ${mediaResponse.status}.`,
	);
}
