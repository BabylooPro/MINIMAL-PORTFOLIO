...
function parsePortfolioDate(value: string): Date {
	...
	if (/^\d{4}-\d{2}$/.test(value)) {
		...
		if (month >= 1 && month <= 12) return new Date(`${value}-01T00:00:00Z`);
	}
...
export function formatPortfolioDate(locale: Locale, value: string, precision: DatePrecision): string {
	const formatter = new Intl.DateTimeFormat(
		localeConfigs[locale].intlLocale,
		precision === "year" ? { year: "numeric", timeZone: "UTC" } : { month: "short", year: "numeric", timeZone: "UTC" },
	);
...
export function isValidPortfolioDate(value: string, precision: DatePrecision): boolean {
	if (precision === "year" && !/^\d{4}$/.test(value)) return false;
