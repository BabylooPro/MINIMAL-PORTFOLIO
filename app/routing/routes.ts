...
export function getRouteFromPathname(pathname: string): AppRoute {
	...
	if (locale && page) return { kind: "legal", locale, page };

	return locale && segments.length === 1 ? { kind: "locale", locale } : { kind: "not-found", locale: locale ?? defaultLocale };
}
...
export function getRouteTitle(route: AppRoute, dictionary: Dictionary): string {
	if (route.kind === "legal") return `${dictionary.messages.legalPages[route.page].title} | ${dictionary.portfolio.name}`;
	return route.kind === "not-found" ? `${dictionary.messages.notFound.title} | ${dictionary.portfolio.name}` : dictionary.messages.meta.title;
}
